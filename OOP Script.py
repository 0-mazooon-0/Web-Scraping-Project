...
class Scraping_Images:
    def __init__(self, topic : str,*, download_path : str = None):
        ...
        if len(topic) < 1 or type(topic) != str:
            raise ValueError("Your topic is too short or invalid please try another one.\n")
        else:
            self.topic = topic
        ...
        if download_path != None:
            if os.path.isdir(os.path.dirname(os.path.abspath(download_path))):
                ...
                raise ValueError("Your driver address is invalid or typed incorrectly please try again.\n")
    ...
    def scrape(self):
        ...
        self.__window.get(f"https://www.google.com/search?sxsrf=AE3TifOzKmAS32r4SRbjNst0HfZz4wyVsw:1754136543040&udm=2&q={self.topic}")
        

        self.__main_code = self.__window.find_element(By.XPATH, '//*[@id="rso"]/div')

        last_height = 0
        while 1:
            new_height = self.__window.execute_script("return document.body.scrollHeight")
            self.__window.execute_script(f"window.scrollTo({last_height}, document.body.scrollHeight);")
            t.sleep(2)
        ...
        self.__photo_tags = self.__main_code.find_elements(By.XPATH, '//*[@id="rso"]/div/div/div[1]/div/div/div/div[2]/h3/a/div/div/div/g-img/img')
        self.__photo_src = []
